...
const downloadResume = async (application) => {
  state.downloadingApplicationId = application.id;
  ...
  try {
    ...
    link.download = `${application.user.name}-resume.pdf`;
    ...
    if (error.response?.status === 403) {
      toast.error('You are not allowed to download this resume.');
    ...
    state.downloadingApplicationId = null;
  }
};
...
<template>
  <section class="mx-auto max-w-5xl px-6 py-10">
    ...
    <div v-else class="mt-8 space-y-4">
      <div v-for="application in state.applications" :key="application.id"
        class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
        <RouterLink :to="`/users/${application.user.id}/profile`"
          class="text-xl font-bold text-slate-900 hover:text-indigo-600">
        ...
        <div class="mt-6 flex gap-3">
          <button @click="updateStatus(application, 'accepted')" :disabled="state.updatingApplicationId === application.id ||
          ...
          <button @click="updateStatus(application, 'rejected')" :disabled="state.updatingApplicationId === application.id ||
          ...
          <button @click="downloadResume(application)" :disabled="state.downloadingApplicationId === application.id"
            class="rounded-lg border border-indigo-600 px-4 py-2 font-semibold text-indigo-600 transition hover:bg-indigo-50 disabled:cursor-not-allowed disabled:opacity-50">
            {{
              state.downloadingApplicationId === application.id
                ? 'Downloading...'
                : 'Download CV'
